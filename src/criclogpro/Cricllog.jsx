...
export const Testimonials = () => {
  ...
  return (
    <div className="bg-orange-400 py-10 px-4">
      <h2 className="text-center text-[#4A2EB0] text-2xl font-bold mb-8">Testimonials</h2>
      <div className="max-w-4xl mx-auto">
        <Slider {...settings}>
          {testimonials.map((testimonial, index) => (
            ...
            >
              <p className="text-black italic mb-6">"{testimonial.text}"</p>
              <div className="flex flex-col items-center">
                ...
                <p className="font-semibold text-black">
                  {testimonial.name} <span className="text-black">({testimonial.location})</span>
                </p>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="w-full min-h-[400px] bg-[#00FFCF] flex flex-col justify-center items-center gap-6 py-12 px-4">
      ...
      <div className="w-full flex flex-col md:flex-row justify-center items-center gap-4 md:gap-6">
        ...
        <button className="text-[12px] sm:text-[14px] md:text-[16px] font-bold p-2 border border-black rounded-md hover:bg-white hover:border-white hover:scale-110 hover:text-[#4D28D4] transition duration-200 ease-in-out">
          BECOME PRO TODAY
        </button>
      </div>
    </div>
...
const CricBabaCarousel = () => {
  const videos = [
    {
      id: 1,
      src: " https://youtu.be/EbcH5X0q0qg",
      title: "Lost the match because of one dropped catch?",
    },
    {
      id: 2,
      src: "https://youtu.be/Rop69ZYrOZU",
      title: "Always batting first after winning the toss?",
    },
    {
      id: 3,
      src: "https://youtu.be/03sVmDXSk4U",
      title: "Strategy kya hai Captain?",
    },
    {
      id: 4,
      src: " https://youtu.be/Rop69ZYrOZU",
      title: "Wining the toss",
    },
  ];
  ...
  return (
    <div className="container mx-auto my-10 px-4">
      ...
      <div className="relative w-full max-w-4xl mx-auto overflow-hidden">
        ...
        >
          {videos.map((video) => (
            <div key={video.id} className="w-full flex-shrink-0 px-2">
              ...
              <p className="text-center text-sm sm:text-lg font-medium mt-4">
                {video.title}
              </p>
            </div>
          ))}
        ...
        <button
          onClick={handlePrev}
          className="absolute top-1/2 left-2 sm:left-4 transform -translate-y-1/2 bg-gray-800 text-white rounded-full p-1 sm:p-2 hover:bg-gray-700"
        >
          &#8249;
        </button>
        <button
          onClick={handleNext}
          className="absolute top-1/2 right-2 sm:right-4 transform -translate-y-1/2 bg-gray-800 text-white rounded-full p-1 sm:p-2 hover:bg-gray-700"
        >
          &#8250;
        </button>
